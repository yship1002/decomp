...
def const_model():
    ...
    builder = StoModelBuilder('pyomo', name='pure_binary_x', m_type='MILP', hint=False)
    ...
    def _obj(m, s):
        return m.prob[s] * (
            -1.5 * m.y1 - 4 * m.y2 - 16 * m.x1[s] - 19 * m.x2[s] - 23 * m.x3[s] - 28 * m.x4[s]
        )
