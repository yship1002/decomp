...
from src.main import _check_set, _flatten
...
class TestUtilFunctions(TestCase):
    ...
    def test_check_set_extra_idx(self):
        ...
        with self.assertRaises(IndexError):
            _check_set(_ori_set, _new_dict)

    def test_check_set_missing_idx(self):
    ...
    def test_check_set_empty_old_set(self):
    ...
    def test_check_set_empty_new_dict(self):
    ...
    def test_flatten_var(self):
        ...
        _res_list = [_m.A[1], _m.A[2], _m.A[3]]

        self.assertEqual(_res_list, _flatten([_m.A]))

    def test_flatten_multi_indexed_var(self):
        _m = ConcreteModel()
        _m.A = Var([1, 2], [3, 4])

        _res_list = [_m.A[1, 3], _m.A[1, 4], _m.A[2, 3], _m.A[2, 4]]

        self.assertEqual(_res_list, _flatten([_m.A]))


    def test_flatten_multi_vars(self):
        _m = ConcreteModel()
        _m.A = Var([1, 2, 3])
        _m.B = Var([1, 2, 3])

        _res_list = [_m.A[1], _m.A[2], _m.A[3], _m.B[1], _m.B[2], _m.B[3]]

        self.assertEqual(_res_list, _flatten([_m.A, _m.B]))

    def test_flatten_constraint(self):
        _m = ConcreteModel()
        _set = [1, 2, 3]
        _m.x = Var(_set)
        def con(m, i):
            return m.x[i] == 0
        _m.A = Constraint(_set, expr=con)
